...
start_sec=$(grep "\* Start date:" gpu_id_*.out | awk '{ split($0, parts, "\\* Start date: "); print parts[2];}' | ${dir}/convert_date.py | date -f - +%s | sort | head -1)
end_sec=$(grep "\* End date:" gpu_id_*.out | awk '{ split($0, parts, "\\* End date: "); print parts[2];}' | ${dir}/convert_date.py | date -f - +%s | sort | tail -1)
...
error_count=$(grep -vf ${dir}/error_exclude.txt gpu_id_*.out | grep -i error | wc -l)
